...
// Testes end-to-end do sistema de recomendação musical com memória
describe('Chat de Recomendação Musical - Testes E2E', () => {
  let graph: any;
  let preferencesService: any;
  const testDbPath = './test-preferences.db';

  // Setup antes de todos os testes: cria banco limpo
  before(async () => {
  ...
  // Cleanup após todos os testes: fecha conexão e remove banco
  after(async () => {
  ...
  // Teste 1: Extração e salvamento de preferências
  it('Deve extrair e salvar preferências do usuário', async () => {
    ...
    // Envia mensagem com nome e preferências musicais
    ...
    // Verifica se a resposta reconhece as preferências
    ...
    // Aguarda salvamento assíncrono
    await new Promise(resolve => setTimeout(resolve, 500));

    // Verifica se as preferências foram salvas no banco
  ...
  // Teste 2: Múltiplas trocas e sumarização
  it('Deve manter múltiplas trocas e fazer sumarização', async () => {
    ...
    // Segunda mensagem: adiciona mais preferências
    ...
    // Verifica se todas as preferências foram salvas
    const savedPreferences = await preferencesService.getSummary(userId);
  ...
  // Teste 3: Recuperação de contexto em nova sessão
  it('Deve recuperar contexto em nova sessão', async () => {
    ...
    // Envia informações completas do usuário
    await graph.invoke(
      {
    ...
    // Verifica se todas as informações foram salvas
    const savedPreferences = await preferencesService.getSummary(userId);
  ...
  // Teste 4: Perguntas simples sem extração de preferências
  ...
  // Teste 5: Manutenção do histórico da conversa
  it('Deve manter histórico da conversa', async () => {
    ...
    // Segunda mensagem: pede recomendação
    const response2 = await graph.invoke(
      {
    ...
    // Verifica se o histórico contém ambas as mensagens
  ...
  // Teste 6: Compartilhamento de preferências entre threads do mesmo usuário
  it('Deve compartilhar preferências entre múltiplas threads do mesmo usuário', async () => {
    ...
    // Verifica se as preferências foram compartilhadas
    const savedPreferences = await preferencesService.getSummary(userId);
    ...
    // Verifica que a thread 2 tem acesso ao contexto da thread 1
  ...
  // Teste 7: Conversa longa com múltiplas mensagens
  it('Deve manter histórico da conversa', async () => {
    const userId = 'test-taylor';
    const threadId = `${userId}-${Date.now()}`;
    const config = {
    ...
    // Simula uma conversa longa com várias mensagens
    await graph.invoke(
      {
        messages: [
          new HumanMessage('Oi, sou Taylor e adoro música pop'),
          new HumanMessage('Sou o Leonardo!'),
          new HumanMessage('33 anos'),
    ...
    // Teste apenas verifica que não houve erro com múltiplas mensagens
  });
});